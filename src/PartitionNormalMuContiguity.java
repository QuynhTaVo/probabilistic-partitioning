...
public class PartitionNormalMuContiguity extends Partition<DistributionNormalMuComparable> {

    private HashSet<Pair<String>> neighborSet;
    private double theta = 1;

    public PartitionNormalMuContiguity(String filepath) {
    ...
    private boolean isMerging(double capacity) {
        ...
        while (iterator.hasNext()) {
            entry = iterator.next();
            if (!entry.pair.id1.equals(entry.pair.id2) && this.neighborSet.contains(new Pair<>(entry.pair.id1, entry.pair.id2)) && DistributionNormal.merge(DistributionNormal.merge(entry.ij, entry.ii), entry.jj).cdf(capacity) >= this.theta) {
